...
| `shared/modulio-antraste.css` | Modulio antraštė ir skirtukų juosta (`.app-header`, `.brand-*`, `.lang-toggle`, `.tabbar`, `.tab`) moduliams su tamsia antrašte: PP-tiekejams, PP-esg, PP-carbon (`index.html` ir `epd.html`). Jungiama iškart po `epso-g.css` ir PRIEŠ modulio `<style>` - modulis tik papildo (antraštės mygtukai, dešinė pusė, savi lūžio taškai). Iki 2026-09-25 tos pačios taisyklės buvo keturiose kopijose. Grupės eilutė - `--color-green-15`: smaragdas-50 ant gradiento telefone turėjo 3,2-4,1:1. Telefone (<= 640 px) antraštė slenka, skirtukai limpa prie viršaus (anksčiau PP-esg juos uždengdavo, PP-carbon palikdavo 71 px tarpą). Savo `.brand-group` moduliuose nerašyk (testai tikrina); modulis su savo `sw.js` įrašo šį failą į `SAVI` |
