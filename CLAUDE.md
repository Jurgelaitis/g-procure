...
| Folder | Paskirtis |
|---|---|
| `PP-home` | Portalo puslapiai gyvena SVETAINĖS ŠAKNYJE (`index.html`, `G-Procure_About.html`, `G-Procure_Project.html`, `atsargine-kopija.html`), nes `g-procure.com/` turi būti tikras pradžios puslapis. `PP-home/` liko tik trys nukreipimo failai seniems adresams - naujo turinio ten nedėk. Šaknyje ir `pirkimu-korteles.html` - bendros pirkimo kortelės puslapis (A1, bandomasis, `noindex`, į žemėlapį ir portalą dar neįtrauktas; žr. 4 sk. `shared/pirkimo-kortele.js`) |
| `PP-plan` | Metinio pirkimų plano (MPP) analizė ir grupės centralizavimas. „AI centralizavimas“ skirtuke - algoritmas naršyklėje (BVPŽ CPV5 ir Jaccard), ne dirbtinis intelektas; planas saugomas tik naršyklėje. Testai - `PP-plan/testai.html` (10, nuo 2026-09-26) |
...
| `PP-protocol` | Pirkimų komisijos sprendimų centras (protokolai, pranešimai, auditas). Nuo 2026-09-26 protokolo Word - su bendru `GP_DOCX_STILIAI` (iki tol vienintelis naudojo `styles`), nepavykus protokolui pranešimai nebekuriami, nepavykę pranešimų failai sakomi juosta. Testai - `PP-protocol/testai.html` (6) |
| `PP-negotiation` | Derybų pasirengimo įrankis (BATNA/ZOPA, MEAT). Testai - `PP-negotiation/testai.html` (8, nuo 2026-09-26) |
...
- **Pirkimo kortelė (A1, 2026-09-26, bandomoji):** pirkimo duomenys (pavadinimas, vertė, būdas, vykdytojas,
  ...
  juosta prieš `GP_DARBAS`, testai su `?saugykla=testine`.
...
- Pirkimo kortelės (`gprocure.korteles`, nuo 2026-09-26) saugomos tik naršyklėje ir patenka į atsarginę kopiją.
  Jas įrašo TIK kortelių puslapis `pirkimu-korteles.html`, žmogui paspaudus „Išsaugoti kortelę“; PP-qual ir PP-graphs
  kortelę tik skaito ir savo darbo toliau nesaugo. Kortelėje gali būti asmenų vardai (iniciatorius, organizatorius) -
  kaip ir PP-plan bei PP-protocol įrašuose.
